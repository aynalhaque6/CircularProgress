package com.codecollection.circularprogress;
...
public class ProgressButton extends FrameLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        tapImage.setOnTouchListener(new OnTouchListener() {
            @Override
            ...
            public void run() {
                if (isPressed) {
                    if (progressStatus < 100) {
                        progressStatus++;
                        progressBar.setProgress(progressStatus);
                        handler.postDelayed(this, 50); // Adjust the delay for speed
                    } else {
                        // Progress completed, show toast and trigger listener
